    ...
    def __init__(
        ...
        for i, out_channels in enumerate(hidden_channels):
            ...
            if layer_normalization:
                self._cnn_layers.add_module('layer_normalization',
                                            nn.LayerNorm(out_channels))

            if hidden_nonlinearity:
                self._cnn_layers.add_module('non_linearity',
            ...
            if max_pool:
                ...
                self._cnn_layers.add_module('max_pooling', pool)
...
def _check_spec(spec, image_format):
    ...
    if (output_space is not None
            and not (hasattr(output_space, 'low')
                     and len(output_space.low.shape) == 1)):
        raise ValueError(
    ...
    if image_format == 'NCHW':
        in_channels = spec.input_space.low.shape[0]
        ...
        in_channels = spec.input_space.low.shape[2]
    else:
        raise ValueError(
